...
namespace DevOpsLazar.Q_Shooters
{
    ...
    public class MagReload : AmmoTracker
    {
        ...
        {
            if (shooter.state != Q_Shooter.STATE_IDLE || !Utilities.IsValid(magReceiver.attachedMag))
            {
                return false;
            }
            return true;
        ...
        {
            if (chamberCapacity <= 0 || !Utilities.IsValid(shooter) || !shooter.sync.IsLocalOwner() || shooter.state == Q_Shooter.STATE_DISABLED)
            {
                return false;
            }
