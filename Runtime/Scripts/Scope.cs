...
namespace DevOpsLazar.Q_Shooters
{
    ...
    public class Scope : SmartObjectSyncListener
    {
        ...
        public static void SetupScope(Scope scope)
        {
            ...
            Q_Shooter parentShooter = scope.GetComponentInParent<Q_Shooter>();
            if(!Utilities.IsValid(parentShooter))
            {
                Helper.ErrorLog(scope, "Scope is not a child of a Q-Shooter");
                return;
            }
        ...
        public override void PostLateUpdate()
        {
            ...
            nextADS = Input.GetKey(zoomShortcut) && shooter.state != Q_Shooter.STATE_RELOAD;
            if (nextADS != ADS)
            {
                recordStartZoomTransforms();
            }
