...
{
    public class Q_ShootersPlayerHandler : UdonSharpBehaviour
    {
        [HideInInspector]
        ...
        public static bool SetupPlayers()
        {
            ...
            Q_ShootersPlayerHandler[] handlers = GameObject.FindObjectsOfType<Q_ShootersPlayerHandler>();
            if (handlers.Length > 1)
            {
                Helper.ErrorLog(handlers[0],"Multiple Q-Shooter Player Handlers found in scene. There should only be one.");
                return false;
            }
            if (handlers.Length == 0)
            {
                ...
                {
                    Helper.ErrorLog(listeners[0],"Player Listeners were found in scene, but no Q-Shooters Player Handler was found. Listeners won't have any effect without any player handlers");
                    return true;
                }
                return true;
            }
            ...
            {
                Helper.ErrorLog(handlers[0],"Q-Shooter Player Handlers or Player Listeners found in scene, but no player objects were found. Did you forget to add a player object pool?");
                return true;
            }
            if (!Helper.IsEditable(handlers[0]))
            {
                Helper.ErrorLog(handlers[0],"Q-Shooter Player Handler is not editable");
                return true;
            }
