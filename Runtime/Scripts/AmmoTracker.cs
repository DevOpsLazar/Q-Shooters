...
{
    [UdonBehaviourSyncMode(BehaviourSyncMode.Manual), RequireComponent(typeof(Q_Shooter))]
    public abstract class AmmoTracker : UdonSharpBehaviour
    {
        [HideInInspector]
        public Q_Shooter shooter;
        ...
        public static void SetupAmmoTracker(AmmoTracker ammoTracker)
        {
            ...
            obj.FindProperty("shooter").objectReferenceValue = ammoTracker.GetComponent<Q_Shooter>();
            obj.ApplyModifiedProperties();
            if (ammoTracker.shooter == null)
            {
                Helper.ErrorLog(ammoTracker, "AmmoTracker is missing a Q_Shooter");
                return;
            }
